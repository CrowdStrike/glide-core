...
// a subset of event properties. These should cover almost all of our use cases.
...
export const test = baseTest.extend<{
  ...
  setProperty: (locator: Locator, name: string, value: string) => Promise<void>;
}>({
  // Playwright throws if the first argument isn't destructured.
  //
    ...
    await use((locator: Locator, event, options) => {
      return locator.evaluate(
        async (node, { event, options }) => {
          ...
          // Wait a ticket to catch any duplicate events, which we test against.
  ...
  async mount({ page }, use) {
    await use(async function mount(template: TemplateResult): Promise<void> {
      ...
      // Stop the browser from attempting to fetch a favicon so there's not a 404 in the
      // console.
      ...
      const components = await page.evaluate((html) => {
        document.body.innerHTML = html;

        return [...document.body.querySelectorAll('*')]
          .filter((element) => element.tagName.startsWith('GLIDE-CORE'))
      ...
      await Promise.all(
        components.map((component) => {
          ...
          // Digging through the manifest is only necessary because sub-component name don't
          // map directly to filenames. For example, `glide-core-dropdown-option` maps to
          // `dropdown.option.ts` on disk instead of mapping to `dropdown-option.ts`.
          //
          ...
          for (const module of customElements.modules) {
            const exportedComponent = module.exports.find(({ kind, name }) => {
              return kind === 'custom-element-definition' && name === component;
            });
          ...
          if (!url) {
            throw new Error(
              `"${component}" wasn't found in \`custom-elements.json\`. Does the component exist? If so, is the manifest up to date? \`pnpm start\` will update the manifest.`,
            );
          }
    ...
    await use((locator: Locator, name: string, value: string) => {
      return locator.evaluate(
        (element, options) => {
          ...
          element[options.name] = options.value;
        },
        { name, value },
      );
    });
