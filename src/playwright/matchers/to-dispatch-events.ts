...
export default expect.extend({
  async toDispatchEvents(
    ...
    await locator.evaluate((element, expectedEvents) => {
      ...
      // A Set is used to deduplicate events so only one listener is added per event
      // type. Duplicating listeners
    ...
    await new Promise((resolve) => {
      ...
      // to run, especially in CI. Try increasing this number slightly if you see the
      // a test passing locally but failing in CI. Most likely not enough time has
      // passed for the listen to be called after an action.
      //
