...
export default class Options extends LitElement {
  ...
  override connectedCallback() {
    ...
    // TODO: is slot check needed?
    const parentElement =
      this.parentElement instanceof HTMLSlotElement
        ? this.parentElement.parentElement
        : this.parentElement;

    const isParentMenu = parentElement instanceof Menu;
    const isParentSelect = parentElement instanceof Select;

    // TODO: explain web driver bit
    if (!isParentMenu && !isParentSelect && !window.navigator.webdriver) {
      throw new Error(
        "Options is only meant for use in Menu or Select. Please talk to us if Menu or Select doesn't meet your needs.",
      );
    }
