...
it('sets `aria-valuemin` on the thumb as `min`', async () => {
  const host = await fixture<GlideCoreSlider>(
  ...
  expect(singleThumb?.getAttribute('aria-valuemin')).to.equal('20');
});
...
it('sets `aria-valuemax` on the minimum thumb as `max`', async () => {
  ...
  expect(singleThumb?.getAttribute('aria-valuemax')).to.equal('200');
});

it('sets `aria-valuenow` on the thumb as `value`', async () => {
  const host = await fixture<GlideCoreSlider>(
  ...
  expect(singleThumb?.getAttribute('aria-valuenow')).to.equal('7');
});
...
it('updates all relevant elements when `max` is set programmatically', async () => {
  ...
  expect(singleThumb?.getAttribute('aria-valuemax')).to.equal('75');
});
...
it('updates all relevant elements when `min` is set programmatically', async () => {
  ...
  expect(singleThumb?.getAttribute('aria-valuemin')).to.equal('25');
});
...
it('decreases by `step` on ArrowLeft on the thumb', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider label="Label" step="10"></glide-core-slider>`,
  );
...
it('decreases by `step` on ArrowDown on the thumb', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider label="Label" step="10"></glide-core-slider>`,
  );
...
it('increases by `step` on ArrowRight on the thumb', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider label="Label" step="10"></glide-core-slider>`,
  );
...
it('increases by `step` on ArrowUp on the thumb', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider label="Label" step="10"></glide-core-slider>`,
  );
...
it('decreases by x10 `step` on PageDown on the thumb', async () => {
  const host = await fixture<GlideCoreSlider>(
...
it('increases by x10 `step` on PageUp on the thumb', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider label="Label" step="2"></glide-core-slider>`,
  );
...
it('sets the value to `min` on Home on the thumb', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider label="Label"></glide-core-slider>`,
  );
...
it('sets the value to `max` on End on the thumb', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider label="Label"></glide-core-slider>`,
  );
...
it('decreases by `step` when dragging the thumb', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider label="Label" step="10"></glide-core-slider>`,
  );
...
it('increases by `step` when dragging the thumb', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider label="Label" step="10"></glide-core-slider>`,
  );
...
it('clicking on the track updates the value', async () => {
  ...
  // Verify the thumb has moved to the clicked position
  const singleThumb = host.shadowRoot?.querySelector(
    '[data-test="single-thumb"]',
  );
  ...
  const thumbStyle = window.getComputedStyle(singleThumb);
  const thumbLeft = Number.parseFloat(thumbStyle.left);
  const thumbLeftPercent = (thumbLeft / trackRect.width) * 100;

  // The thumb should be positioned at approximately 60% (allowing for small rounding differences)
  expect(thumbLeftPercent).to.be.closeTo(60, 2);
});
...
it('sets the thumb to aria-disabled when `disabled`', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider label="Label" disabled></glide-core-slider>`,
  );
  ...
  expect(singleThumb?.hasAttribute('aria-disabled')).to.be.true;
});

it('sets the thumb to aria-readonly when `readonly`', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider label="Label" readonly></glide-core-slider>`,
  );
  ...
  expect(singleThumb?.hasAttribute('aria-readonly')).to.be.true;
});

it('does not update when dragging the thumb when `disabled`', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider
      label="Label"
      .value=${[25]}
...
it('does not update when dragging the thumb when `readonly`', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider
      label="Label"
      .value=${[25]}
...
it('prevents updating the `value` when using the arrow keys on the thumb when `disabled`', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider
      label="Label"
      .value=${[25]}
...
it('prevents updating the `value` when using the arrow keys on the thumb when `readonly`', async () => {
  const host = await fixture<GlideCoreSlider>(
    html`<glide-core-slider
      label="Label"
      .value=${[25]}
...
it('hides the minimum and maximum inputs and sliders', async () => {
  ...
  expect(maximumThumb?.checkVisibility()).to.not.be.ok;
  expect(minimumThumb?.checkVisibility()).to.not.be.ok;
});
