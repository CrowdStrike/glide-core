...
export default class Menu extends LitElement {
  ...
  // additionally uses this field to signal to sub-Menus that they've been opened via keyboard.
  ...
  override connectedCallback() {
    ...
    // seems prudent to only handle document clicks for the top-level Menu.
    //
    ...
    if (!this.#isSubMenu) {
      ...
      // 3. That handler is called. It sets `open` to `true`.
      ...
      // `true` in the consumer's handler isn't overwritten by this handler setting it
      // to `false`.
  ...
  override firstUpdated() {
    ...
    if (this.open && !this.isTargetDisabled) {
      ...
      for (const subMenu of this.#subMenus) {
        if (subMenu.open) {
          ...
          // level Menu isn't or its target is disabled.
          //
          // What if we keep one open and the top-level Menu isn't initially open? Then, when
          // the top-level Menu is opened, it'll appear on top of the sub-Menu because the
          // top-level Menu was opened second. So we'd have to toggle the sub-Menu's closed
          // then open.
          //
          // But what is the use case for having a sub-Menu open when the top-level Menu isn't
          // open? It's possible one exists. until such a case presents itself, however, closing
          ...
          // how nested sub-Menus get closed. They're closed via `#hide()`, which necessarily
          // closes sub-Menus when a Menu is hidden.
          subMenu.open = false;
        }
      }
    }
    ...
    if (this.#defaultSlotElementRef.value) {
      // `popover` is used so Options can break out of Modal or another container that has
      // `overflow: hidden`. Elements with `popover` are positioned relative to the viewport.
      // Thus Floating UI in addition to `popover`.
      //
      // "manual" is set here instead of in the template to circumvent Lit Analyzer, which
      // isn't aware of `popover` and doesn't have a way to disable its "no-unknown-attribute"
      // rule.
      //
      // "manual" instead of "auto" because the latter only allows one popover to be open at
      // a time. And consumers may have popovers in own components that need to be open while
      // this one is open.
      //
      // "auto" also automatically opens the popover when its target is clicked. We want it
      // to remain closed when clicked when there are no Options or the target is disabled.
  ...
  override render() {
    // The linter wants a "focus" handler on the slot and "focusin" apparently doesn't
    // satisfy it.
    //
  ...
  // events will be retargeted to the light DOM the moment they bubble out of Menu. So
  ...
  // even if the user mouses down on Menu's target then moves his mouse outside of
  // Menu before mousing up.
  ...
  #onComponentFocusIn(event: Event) {
    ...
    // VoiceOver again. If VoiceOver focused an Option, we make it active so it's displayed
    // as such and so the correct Option is selected on Enter or Space.
    if (
  ...
  #onDefaultSlotClick(event: Event) {
    ...
    //
    // The event will then get picked by this handler in the super-Menu, and that
    // Menu will close because `event.target` will be an Option.
    //
    // Stopping propagation of the event at the sub-Menu prevents it from being handled
    // by the super-Menu and the super-Menu stays open.
    if (this.#isSubMenu && event.target === this.#defaultSlotElementRef.value) {
      event.stopPropagation();
    }

    // The timeout gives consumers a chance to cancel the event, preventing Menu from
    // closing.
    setTimeout(() => {
      if (
        !event.defaultPrevented &&
        // Checked because clicks can come from sub-Menu targets, arbitrary content in the
        // default slot, and the default slot's border or padding.
        //
        ...
        // can listen for clicks on the arbitrary content and close Menu if they need to.
        //
        // When the default slot's padding is clicked, Menu should remains open because the
  ...
  #onDefaultSlotMouseDown(event: Event) {
    // So clicking an Option doesn't move focus to `document.body`.
    //
    // Imagine a case where the user opens Menu by clicking its target then clicks an
    // Option. The user should be able immediately to press Space or Enter afterward
    // to reopen Menu. It's not uncommon users to change modalities when interacting
    // with something.
  ...
  #onDefaultSlotMouseOver(event: MouseEvent) {
    ...
    // milliseconds, rendered above where it's supposed to be when `showPopover()`
    // is called.
    //
    ...
    // invisible and still picks up "mouseover" events.
    //
    // Either way, if the user's mouse happens to be over one of the Option's that inside
    // our invisible, mispositioned popover, then the Option is activated. Thus this
    // variable and the guard below.
    //
    ...
    if (!isOutOfBounds) {
      ...
      if (this.#isSubMenu) {
        // Allowing the event to propagate from a sub-Menu to a super-Menu Option means
        // it would get picked up by the super-Menu Option's Tooltip "mouseover" handler
        // and then open the super-Menu's tooltip.
        event.stopPropagation();
      }
      ...
      if (isSubMenuTarget && this.#activeOption) {
        ...
        // tooltip will be closed then immediately reopened. Yet we want the tooltip to close
  ...
  #onDefaultSlotToggle(event: Event) {
    ...
    if (event.target instanceof Menu) {
      for (const subMenu of this.#subMenus) {
        ...
        // Menu can have more than one Option with a sub-Menu. If the event was a result of
        // a sub-Menu being opened and another sub-Menu is already open, we make sure to close
        // the already open sub-Menu.
        //
        // As you'd expect, nested sub-Menus also dispatch "toggle" events when they're opened.
        // So we use `isOwnSubMenu` to guard against closing our own open sub-Menu when a nested
        // sub-Menu is opened.
        if (
          isOwnSubMenu &&
  ...
  #onTargetAndDefaultSlotKeyDown(event: KeyboardEvent) {
    ...
    if (isArbitraryContent) {
      // Arbitrary interactive content, either at the top of bottom of Menu, isn't a great
      ...
      // the content exists until they tab past Menu's target. Nonetheless, it's a pattern
      // we need to support.
      //
      ...
      // himself. But there's a good chance he won't think to. So we handle do so for him.
    ...
    // If all of the below conditions are met, then the event came from a sub-Menu,
    // in which case the state of this Menu shouldn't change. So we return.
    //
    // Returning also prevents a loop where the event is redispatched on the sub-Menu
    // then picked up again by its this Menu and on and on.
    ...
    if (this.#isSubMenuOpen) {
      ...
      // the intends to interact with.

      // Normally this rule makes sense. But here we're not dispatching an event for consumers,
      // so it doesn't belong in the JSDoc comment. We're simply redispatching an event from
      // the user.
      //
      ...
      //    the insertion point from moving in addition to previous Option becoming active.
      //
      ...
      //    the insertion point from moving in addition to the next Option becoming active.
      //
      ...
      //
      // Below, each super-Menu checks if the event that was dispatched on its sub-Menu
      ...
      // only knows that one of its own sub-Menus is open and so redispatches the event.
      // It doesn't know what sub-Menu Option is active
      //
      // Imagine a sub-Menu is open and the sub-Menu's active Option has a sub-Menu. Now
      // the user presses ArrowRight to open that sub-Menu's sub-Menu. The top-level Menu
      // doesn't know if ArrowRight will open a nested sub-Menu or not. That means, in
      // situation #1, it doesn't know if it should cancel the event to prevent the insertion
      // point from moving.
      //
      // So we let sub-Menus communicate to the top-level Menu whether its own event should
      // be canceled. While convoluted to explain in writing, the approach is quite simple
      // in practice and preserves boundaries between a Menu and its sub-Menus.
      if (subevent.defaultPrevented) {
        event.preventDefault();
      }

      // This is the one case where both a super-Menu and sub-Menu both handle an event
      // because its the one case where two things need to happen. The sub-Menu needs to
    ...
    if (this.open && event.key === ' ' && !this.#isFilterable) {
      ...
      // when Menu is closed after the active Option is clicked below. We move focus
      // to the target instead so the VoiceOver user isn't kicked to the top of the page.
      if (this.#hasVoiceOverMovedFocusToOptionsOrAnOption && !this.#isSubMenu) {
        this.#focus();
      }
  ...
  #onTargetSlotChange() {
    ...
    if (this.#targetElement && this.#optionsElement) {
      ...
      // sure the target set up correctly.
      //
      ...
      // interacted with via keyboard. That's also why we set `tabIndex` below.
  ...
  #onTargetSlotClick(event: PointerEvent) {
    ...
    if (isSubMenuTarget && isClosestOptionALink) {
      ...
      // shouldn't cause a navigation. So we cancel the event. We have to cancel it
    ...
    if (isSubMenuTarget) {
      ...
      // So sub-Menu target clicks are just noise.
      event.stopPropagation();
    }
