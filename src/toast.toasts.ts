...
export default class GlideCoreToasts extends LitElement {
  ...
  override render() {
    ...
    // the "description" slot is only so we can programmatically click the consumer's
    // Toast.
    //
          ...
          (toast) => {
            return html`<div
              ...
              class=${classMap({
                ...
                hide: Boolean(toast.privateHide),
                'via-dismiss-button': Boolean(
                  toast.privateHideViaDismissButton,
                ),
              })}
              data-test="toast"
              role="alert"
  ...
  static async privateAdd(toast: GlideCoreToast) {
    ...
    if (toast.duration < Number.POSITIVE_INFINITY) {
      toast.privateTimeoutId = setTimeout(() => {
        ...
        if (isReducedMotion) {
          GlideCoreToasts.privateRemove(toast);
        } else {
          toast.privateHide = true;

          // Because `privateHide` is only a reactive property of Toast, Toasts won't
          // know it needs to rerender. So we let it know.
          toasts.requestUpdate();
        }
      }, toast.duration);
  ...
  static privateRemove(toast: GlideCoreToast) {
    const toasts = document.querySelector('glide-core-private-toasts');

    if (toasts) {
      const index = toasts.toasts.indexOf(toast);
  ...
  #onToastDismissButtonClick(toast: GlideCoreToast) {
    ...
    if (isReducedMotion) {
      ...
      toast.privateHide = true;
      toast.privateHideViaDismissButton = true;

      // Because `privateHide` and `privateHideViaDismissButton` are only reactive
    ...
    if (toast.duration < Number.POSITIVE_INFINITY) {
      toast.privateTimeoutId = setTimeout(() => {
        ...
        if (isReducedMotion) {
          ...
          // Because `privateHide` is only a reactive property of Toast, Toasts
  ...
  #onToastTransitionEnd(toast: GlideCoreToast) {
    ...
    // Checking `privateHide` ensures we don't dismiss toasts when "transitionend"
    // comes from the transition when they are shown ending.
    if (toasts && toast.privateHide) {
      GlideCoreToasts.privateRemove(toast);
    }
  }
}
