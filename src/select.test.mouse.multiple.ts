...
test(
  'does not deselect its selected options when another is selected via mouse',
  { tag: '@mouse' },
  async ({ mount, page }) => {
    await mount(
      () => html`
    ...
    await expect(host).toDispatchEvents(
      () => page.getByRole('option').nth(2).click(),
      [
        { bubbles: true, composed: true, type: 'input' },
        { bubbles: true, composed: true, type: 'change' },
      ],
    ...
    await expect(host).toHaveJSProperty('value', ['one', 'two', 'three']);
    await expect(host).toHaveAttribute('open');
