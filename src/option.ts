...
export default class Option extends LitElement {
  ...
  // Consumers may chose not to take the happy path and to instead use the "content"
  // slot. In that case, we don't render `label`. But `label` still needs to be supplied
  // so we can pass it to Tooltip. That's why it's required.
  /**
   * @default undefined
   */
  ...
  set label(label: string) {
    ...
    // Wait for the label to render. A rerender won't be scheduled by Lit until
  ...
  set privateActive(isActive: boolean) {
    ...
    // because the label and description's `scrollWidth` and `clientWidth` will both
    // be zero until Menu is open. Rather than expose a pseudo-private method for Menu
    // to call on open, we simply check for overflow when an option becomes active.
    //
    // TODO: also way way timeout
    setTimeout(() => {
      this.#updateContentSlotOverflow();
    });
  }
  ...
  override render() {
    ...
    // default slot, but provides an "icon" or "submenu" slot, then whitespace will
    // necessarily be introduced into the default slot via the whitespace around that
    // slotted content. And neither the "icon" or "default" slots nor the `label` or
      ...
      <glide-core-tooltip
        ...
        >
          ...
          >
            ...
            ${when(this.selected && this.role === 'option', () => {
              return html`<div
  ...
  // Set in `#onSubmenuToggle()`. Used to toggle the submenu's target as open
  // or closed visually.
  @state()
  ...
  #onSubmenuToggle(event: Event) {
    ...
    // Nested submenus can be opened or closed and the target of the option's
    // own submenu shouldn't change visually. As long as the option's own
    // submenu is open, the submenu's target should appear open. Similar for
    // when a nested submenu is closed.
  ...
  #onTooltipClick(event: Event) {
    if (this.disabled) {
      ...
      // Letting the event propagate would result in a false positive event bubbling
      // up to the consumer. It would also get picked up by Menu, which would
      // close.
    ...
    //
    // `!isTargetClick` because Menu needs to know if a target of a submenu was clicked to
    // avoid closing itself.
    //
    // `!isSubmenuOption` so we don't retarget submenu option clicks to parent their option.
    if (
      !isTooltipClick &&
      !isContainerClick &&
      !isTargetClick &&
  ...
  #onTooltipToggle(event: Event) {
    if (event.target instanceof Tooltip) {
      ...
      // in knowing when this component's Tooltip is open. So it's probably best to stop
