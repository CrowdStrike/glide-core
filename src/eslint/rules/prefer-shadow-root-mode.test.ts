...
test(
  'valid when `shadowRootOptions` has the correct `mode` with a property',
  { tag: '@eslint' },
  async () => {
...
test(
  'invalid when a class with a property has no `shadowRootOptions`',
  { tag: '@eslint' },
  async () => {
    const [result] = await eslint.lintText(`
    class Example extends LitElement {
...
test(
  'invalid when `shadowRootOptions` has no `mode`',
  { tag: '@eslint' },
  async () => {
    const [result] = await eslint.lintText(`
    class Example extends LitElement {
...
test(
  'invalid when existing `shadowRootOptions` has delegatesFocus but no `mode` is specified',
  { tag: '@eslint' },
  async () => {
    const [result] = await eslint.lintText(`
    class Example extends LitElement {
