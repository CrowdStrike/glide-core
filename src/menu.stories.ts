...
// of a submenu, in which case it shouldn't be focusable.
//
    ...
    '<glide-core-option>[slot="content"]': {
      ...
      table: {
        ...
        type: {
          summary: 'any',
          detail: `
// This is the unhappy path. It's the escape hatch for you to render arbitrary content and lay it out hover
// you need to.
//
// If you use this slot, \`slot="icon"\` and \`slot="submenu"\` will become unavailable. And the \`label\` and
// \`description\` attributes won't be rendered.
//
// The \`label\` attribute is still required. We'll show it in a tooltip when your content overflows. If you
// need a second line of text in the tooltip, provide it via the  \`description\` attribute.
`,
        },
      },
      type: { name: 'function' },
