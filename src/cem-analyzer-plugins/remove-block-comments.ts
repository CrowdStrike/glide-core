...
/**
 * Removes existing JSDocs from the AST.
 *
 ...
 *
 * But just because something is in a component's JSDoc doesn't meant it
...
export default () => {
  return {
    ...
    collectPhase({ node }: { node: Node }) {
      ...
      // have comments too. And those we need.
      //
      // The default return types of getters, for example, are specified
      // in JSDocs because they're otherwise difficult to determine.
