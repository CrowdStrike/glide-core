...
it('makes focusable submenu targets unfocusable', async () => {
  const host = await fixture<Menu>(
    html`<glide-core-menu>
      <button slot="target">Target</button>
  ...
  const submenuTarget = host.querySelector('span');
  expect(submenuTarget?.tabIndex).to.equal(-1);
});
...
it('is opened when it and its submenu are open', async () => {
  const host = await fixture<Menu>(
    html`<glide-core-menu open>
      <button slot="target">Target</button>
...
it('closes all but the first open submenu when multiple submenus are open', async () => {
  const host = await fixture<Menu>(
    html`<glide-core-menu open>
      <button slot="target">Target</button>
...
it('does not clobber the existing ID of its target', async () => {
  const host = await fixture<Menu>(
    html`<glide-core-menu>
