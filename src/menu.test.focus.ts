...
it('sets an Option as active when one focused programmatically', async () => {
  const host = await fixture<Menu>(
    html`<glide-core-menu open>
      <button slot="target">Target</button>
...
it('focuses the parent Option of a sub-Menu on Escape when a sub-Menu Option was focused programmatically', async () => {
  const host = await fixture<Menu>(
    html`<glide-core-menu open>
      <button slot="target">Target</button>
