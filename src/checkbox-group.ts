  ...
  implements FormControl
{
  ...
  override firstUpdated() {
    ...
    for (const checkbox of this.#checkboxElements) {
      ...
      // may conflict with the one derived from which checkboxes are selected.
      //
      // So we have a decision to make. On first render, do we defer to the initial
      // `value` and check and uncheck options below? Or do we defer to `#onDefaultSlotChange()`
      ...
      // to check options based on the initial `value` and that the initial `value` is
