...
import { isCallExpression, isClassDeclaration, isExpressionWithTypeArguments, isIdentifier, isNewExpression, isPropertyAccessExpression, isStringLiteral, SyntaxKind, } from 'typescript';
    ...
    return {
        ...
        analyzePhase({ moduleDoc, node }) {
            if (isClassDeclaration(node) && node.heritageClauses) {
                const isImplementsFormControl = isClassDeclaration(node) &&
                    node.heritageClauses.some((clause) => {
                        return clause.types.some((type) => {
                            return (isExpressionWithTypeArguments(type) &&
                ...
                for (const argument of node.arguments) {
                    ...
                    for (const declaration of moduleDoc.declarations) {
                        if ('events' in declaration &&
                            declaration.events &&
                            getParentClassName(node) === declaration.name) {
                            const isNew = declaration.events.every(({ name }) => name !== eventName);
