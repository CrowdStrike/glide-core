...
import { isReturnStatement, isTaggedTemplateExpression, } from 'typescript';
    ...
    return {
        ...
        analyzePhase({ moduleDoc, node }) {
            if (isTaggedTemplateExpression(node) &&
                ...
                */`).at(0);
                    return {
                        name: block?.tags.find((tag) => tag.tag === 'name')?.name ??
                ...
                for (const declaration of moduleDoc.declarations) {
                    if (getParentClassName(node) === declaration.name &&
                        ...
                        for (const slot of slots) {
                            const isNew = declaration.slots.every(({ name }) => name !== slot.name);
