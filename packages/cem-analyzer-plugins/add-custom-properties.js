...
import {} from 'custom-elements-manifest' with { type: 'json' };
import { isClassDeclaration, isImportDeclaration, isStringLiteral, isTaggedTemplateExpression, isTemplateExpression, } from 'typescript';
...
export default () => {
    return {
                    ...
                    if (ruleNode instanceof Rule && ruleNode.selector === ':host') {
                        for (const declarationNode of ruleNode.nodes) {
                            if (declarationNode instanceof Declaration &&
                                ...
                                const fileName = nodePath.basename(node.getSourceFile().fileName);
                                const comment = declarationNode.parent?.nodes.findLast((node) => {
                                ...
                                const customProperty = {
                                    ...
                                    description: comment instanceof Comment ? comment.text : undefined,
                                };
                                const isNew = context.stylesheets[fileName]?.every(({ name }) => name !== customProperty.name);
        ...
        analyzePhase({ context, moduleDoc, node, }) {
            const isComponent = isClassDeclaration(node) &&
            ...
            for (const statement of node.getSourceFile().statements) {
                if (isImportDeclaration(statement) &&
                    ...
                    for (const declaration of moduleDoc.declarations) {
                        if (getParentClassName(node) !== declaration.name ||
                            !context.stylesheets) {
                            return;
                        }
                        ...
                        const fileName = statement.moduleSpecifier.text.slice(0, Math.max(0, statement.moduleSpecifier.text.lastIndexOf('.'))) + '.ts';
                        const stylesheet = context.stylesheets[nodePath.basename(fileName)];
                        if ('cssProperties' in declaration &&
                            declaration.cssProperties &&
                            stylesheet) {
                            for (const property of stylesheet) {
                                const isNew = declaration.cssProperties.every(({ name }) => name !== property.name);
                                if (isNew) {
