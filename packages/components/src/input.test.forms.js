...
it('can be reset to its initial value', async () => {
    ...
    const host = await fixture(html `<glide-core-input label="Label" value="value"></glide-core-input>`, {
        parentNode: form,
    });
    host.value = '';
    form.reset();
...
it('can be reset if there was no initial value', async () => {
    const form = document.createElement('form');
    const host = await fixture(html `<glide-core-input label="Label"></glide-core-input>`, {
        parentNode: form,
    });
    host.value = 'value';
    form.reset();
...
it('has no `formData` value when no value', async () => {
    const form = document.createElement('form');
    await fixture(html `<glide-core-input label="Label" name="name"></glide-core-input>`, {
        parentNode: form,
    });
    const formData = new FormData(form);
...
it('has no `formData` value when it has a value but without a `name`', async () => {
    const form = document.createElement('form');
    await fixture(html `<glide-core-input label="Label" value="value"></glide-core-input>`, {
        parentNode: form,
    });
    const formData = new FormData(form);
    expect(formData.get('name')).to.be.null;
...
it('is valid if empty but not required', async () => {
    ...
    expect(host.shadowRoot?.querySelector('[data-test="input"]')?.ariaInvalid).to.equal('false');
});
it('is valid after being filled in when empty and required', async () => {
    const host = await fixture(html `<glide-core-input label="Label" required></glide-core-input>`);
    await sendKeys({ press: 'Tab' });
...
it('is invalid if no value and required', async () => {
    ...
    await host.updateComplete;
    expect(host.shadowRoot?.querySelector('[data-test="input"]')?.ariaInvalid).to.equal('true');
});
it('is invalid after value is cleared when required', async () => {
    const host = await fixture(html `<glide-core-input
      label="Label"
      value="value"
      clearable
      required
...
it('is valid if no value and required and disabled', async () => {
    const host = await fixture(html `<glide-core-input label="Label" disabled required></glide-core-input>`);
    expect(host.validity?.valid).to.be.true;
    expect(host.validity?.valueMissing).to.be.false;
    expect(host.checkValidity()).to.be.true;
    expect(host.reportValidity()).to.be.true;
...
it('sets the validity message with `setCustomValidity()`', async () => {
    ...
    expect(host.shadowRoot?.querySelector('[data-test="validity-message"]')
