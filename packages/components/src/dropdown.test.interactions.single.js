...
it('shows an Edit button when `editable` of the selected option is set programmatically', async () => {
    ...
    await host.updateComplete;
    const editButton = host.shadowRoot?.querySelector('[data-test="edit-button"]');
...
it('updates its internal label when an option is newly selected', async () => {
    ...
    // Wait for Floating UI.
    await aTimeout(0);
    const option = host.querySelector('glide-core-dropdown-option:last-of-type');
    await click(option);
    const label = host.shadowRoot?.querySelector('[data-test="internal-label"]');
    expect(label?.textContent?.trim()).to.equal(option?.label);
});
it('hides Select All', async () => {
    const host = await fixture(html `<glide-core-dropdown label="Label" select-all>
      <glide-core-dropdown-option
        label="One"
        selected
      ></glide-core-dropdown-option>
...
it('cannot be tabbed to when disabled', async () => {
    await fixture(html `<glide-core-dropdown label="Label" disabled>
      <glide-core-dropdown-option label="One"></glide-core-dropdown-option>
      <glide-core-dropdown-option label="Two"></glide-core-dropdown-option>
    </glide-core-dropdown>`);
