...
it('can be reset if it has an initial value', async () => {
    ...
    const host = await fixture(html `<glide-core-textarea label="Label" value="one"></glide-core-textarea>`, { parentNode: form });
...
it('can be reset if it has no initial value', async () => {
    const form = document.createElement('form');
    const host = await fixture(html `<glide-core-textarea label="Label"></glide-core-textarea>`, { parentNode: form });
    host.value = 'value';
    form.reset();
...
it('is valid by default', async () => {
    const host = await fixture(html `<glide-core-textarea label="Label"></glide-core-textarea>`);
    ...
    expect(host.shadowRoot?.querySelector('[data-test="textarea"]')?.ariaInvalid).to.equal('false');
});
it('is valid after being filled in and required', async () => {
    const host = await fixture(html `<glide-core-textarea label="Label" required></glide-core-textarea>`);
    host.focus();
    await sendKeys({ type: 'value' });
    expect(host.validity?.valid).to.be.true;
    expect(host.validity?.valueMissing).to.be.false;
...
it('is invalid if no value and required', async () => {
    ...
    await host.updateComplete;
    expect(host.shadowRoot?.querySelector('[data-test="textarea"]')?.ariaInvalid).to.equal('true');
});
...
it('submits its form on Meta + Enter', async () => {
    const form = document.createElement('form');
    await fixture(html `<glide-core-textarea label="Label"></glide-core-textarea>`, { parentNode: form });
    const spy = sinon.spy();
