...
it('removes itself on click', async () => {
    ...
    host.click();
    const animationDuration = host.shadowRoot?.querySelector('[data-test="component"]')?.dataset.animationDuration;
...
it('does not remove itself on click when disabled', async () => {
    const host = await fixture(html `<glide-core-tag label="Label" disabled removable></glide-core-tag>`);
    host.click();
    const animationDuration = host.shadowRoot?.querySelector('[data-test="component"]')?.dataset.animationDuration;
    await aTimeout(Number(animationDuration));
