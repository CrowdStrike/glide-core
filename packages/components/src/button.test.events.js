...
it('dispatches a "reset" event on click', async () => {
    ...
    const host = await fixture(html ` <glide-core-button label="Label" type="reset"></glide-core-button>`, {
        parentNode: form,
    });
    click(host);
...
it('dispatches a "reset" event on Enter', async () => {
    const form = document.createElement('form');
    await fixture(html ` <glide-core-button label="Label" type="reset"></glide-core-button>`, {
        parentNode: form,
    });
    await sendKeys({ press: 'Tab' });
    sendKeys({ press: 'Enter' });
...
it('dispatches a "submit" event on click', async () => {
    const form = document.createElement('form');
    const host = await fixture(html ` <glide-core-button label="Label" type="submit"></glide-core-button>`, {
        parentNode: form,
    });
...
it('dispatches a "submit" event on Enter', async () => {
    const form = document.createElement('form');
    await fixture(html ` <glide-core-button label="Label" type="submit"></glide-core-button>`, {
        parentNode: form,
    });
    form.addEventListener('submit', (event) => event.preventDefault());
    await sendKeys({ press: 'Tab' });
