...
it('submits its form on Enter', async () => {
    const form = document.createElement('form');
    const host = await fixture(html `<glide-core-checkbox label="Label"></glide-core-checkbox>`, {
        parentNode: form,
    });
    const spy = sinon.spy();
...
it('is valid but not aria-invalid after being checked when unchecked and required', async () => {
    const host = await fixture(html `<glide-core-checkbox label="Label" required></glide-core-checkbox>`);
