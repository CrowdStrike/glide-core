...
const meta = {
    ...
    decorators: [
        withActions,
        (story) => html `<form action="/" style="width: max-content;">
        <script type="ignore">
    ...
    argTypes: {
        ...
        'addEventListener(event, handler)': {
            ...
            table: {
                type: {
                    summary: 'method',
                    detail: '(event: "change" | "input" | "invalid", handler: (event: Event) => void): void',
                },
            },
        },
        'checkValidity()': {
...
// Clears the validity feedback message and styling while maintaining the state of the component's\n// \`validity\` property.
    ...
    play(context) {
        ...
        if (checkboxGroup instanceof GlideCoreCheckboxGroup) {
            checkboxGroup.addEventListener('change', () => {
                ...
                const checkboxes = context.canvasElement.querySelectorAll('glide-core-checkbox');
                for (const checkbox of checkboxes) {
                    addons.getChannel().emit(UPDATE_STORY_ARGS, {
                        storyId: context.id,
                        updatedArgs: {
                            [`<glide-core-checkbox>.${checkbox.value}.checked`]: checkboxGroup.value.includes(checkbox.value),
                        },
                    });
                }
            });
    ...
    render(arguments_) {
        ...
        >
          <glide-core-checkbox label="One" value="one" ?checked=${arguments_['<glide-core-checkbox>.one.checked']}></glide-core-checkbox>
          <glide-core-checkbox label="Two" value="two" ?checked=${arguments_['<glide-core-checkbox>.two.checked']}></glide-core-checkbox>
          <glide-core-checkbox label="Three" value="three" ?checked=${arguments_['<glide-core-checkbox>.three.checked']}></glide-core-checkbox>
