...
it('focuses the first enabled radio when `focus()` is called', async () => {
    ...
    expect(document.activeElement).to.equal(host.querySelector('glide-core-radio-group-radio:nth-of-type(2)'));
});
...
it('focuses the first radio after `reportValidity()` is called when it is `required` and the radio is unchecked', async () => {
    ...
    host.reportValidity();
    expect(document.activeElement).to.equal(host.querySelector('glide-core-radio-group-radio'));
});
...
it('does not focus a radio after `checkValidity()` is called', async () => {
    ...
    host.checkValidity();
    expect(document.activeElement).to.not.equal(host.querySelector('glide-core-radio-group-radio'));
});
it('focuses the first enabled radio when none are checked and `focus()` is called', async () => {
    ...
    await sendKeys({ press: 'Tab' });
    const radio = host.querySelector('glide-core-radio-group-radio:nth-of-type(2)');
    expect(radio).to.have.focus;
});
