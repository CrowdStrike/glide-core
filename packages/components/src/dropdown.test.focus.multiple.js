...
it('focuses its primary button when `focus()` is called', async () => {
    ...
    expect(host.shadowRoot?.activeElement).to.equal(host.shadowRoot?.querySelector('[data-test="primary-button"]'));
});
it('focuses its primary button on submit when required and no options are selected', async () => {
    const form = document.createElement('form');
    const host = await fixture(html `<glide-core-dropdown label="Label" multiple required>
      <glide-core-dropdown-option label="Label"></glide-core-dropdown-option>
