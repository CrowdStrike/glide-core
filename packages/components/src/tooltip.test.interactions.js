...
it('passes down certain properties to its container', async () => {
    const host = await fixture(html `<glide-core-tooltip label="One">
      <button slot="target">Target</button>
    </glide-core-tooltip>`);
    await waitUntil(() => {
        return host.querySelector('glide-core-private-tooltip-container');
    });
    host.label = 'Two';
...
it('is open when opened programmatically', async () => {
    const host = await fixture(html `<glide-core-tooltip label="Label">
      <button slot="target">Target</button>
    </glide-core-tooltip>`);
    host.open = true;
...
it('is open when open and enabled programmatically', async () => {
    const host = await fixture(html `<glide-core-tooltip label="Label" open disabled>
      <button slot="target">Target</button>
    </glide-core-tooltip>`);
    host.disabled = false;
    // Wait for Floating UI.
...
it('is not open when open and disabled programmatically', async () => {
    const host = await fixture(html `<glide-core-tooltip label="Label" open>
      <button slot="target">Target</button>
    </glide-core-tooltip>`);
    // Wait for Floating UI.
    await aTimeout(0);
...
it('is not open when opened programmatically and disabled', async () => {
    const host = await fixture(html `<glide-core-tooltip label="Label" disabled>
      <button slot="target">Target</button>
    </glide-core-tooltip>`);
    host.open = true;
    // Wait for Floating UI.
...
it('is visible when tabbed to', async () => {
    ...
    // Wait for Floating UI.
    await aTimeout(0);
    expect(host.shadowRoot?.querySelector('[data-test="tooltip"]')?.checkVisibility()).to.be.true;
});
it('remains closed on hover when disabled', async () => {
    ...
    expect(host.shadowRoot?.querySelector('[data-test="tooltip"]')?.checkVisibility()).to.be.false;
});
...
it('closes when hovered away from', async () => {
    ...
    await hover(host.querySelector('button'));
    tooltip.dataset.closeDelay = '0';
    await hover(host.shadowRoot?.querySelector('[data-test="component"]'), 'outside');
    expect(tooltip.checkVisibility()).to.be.false;
});
...
it('sets `aria-describedby` on its target when not hidden from screenreaders', async () => {
    const host = await fixture(html `<glide-core-tooltip label="Label" screenreader-hidden>
      <button slot="target">Target</button>
    </glide-core-tooltip>`);
...
it('has `set open(isOpen: boolean)` coverage', async () => {
    const host = await fixture(html `<glide-core-tooltip
      label="Label"
      placement="right"
