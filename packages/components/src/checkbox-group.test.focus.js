...
it('focuses the first enabled checkbox when `focus()` is called', async () => {
    ...
    host.focus();
    expect(document.activeElement).to.equal(host.querySelector('glide-core-checkbox:last-of-type'));
});
it('focuses the first checkbox after submit when required and its checkbox is unchecked', async () => {
    ...
    form.requestSubmit();
    expect(document.activeElement).to.equal(host.querySelector('glide-core-checkbox'));
});
...
it('does not focus the input after `checkValidity()` is called', async () => {
    ...
    host.checkValidity();
    expect(document.activeElement).to.not.equal(host.querySelector('glide-core-checkbox'));
});
