...
it('has overflow buttons when overflowing', async () => {
    ...
    `, { parentNode });
    await waitUntil(() => {
        return (host.shadowRoot?.querySelector('[data-test="overflow-start-button"]') &&
...
it('does not have overflow buttons when not overflowing', async () => {
  ...
  `);
    const startButton = host.shadowRoot?.querySelector('[data-test="overflow-start-button"]');
    const endButton = host.shadowRoot?.querySelector('[data-test="overflow-end-button"]');
...
it('disables its overflow buttons on scroll', async () => {
    ...
    const startButton = host.shadowRoot?.querySelector('[data-test="overflow-start-button"]');
    const endButton = host.shadowRoot?.querySelector('[data-test="overflow-end-button"]');
    const tabList = host.shadowRoot?.querySelector('[data-test="tablist"]');
    assert(tabList);
...
it('has no overflow buttons when resized to not overflow', async () => {
    ...
    await waitUntil(() => host.shadowRoot?.querySelector('[data-test="overflow-start-button"]') === null &&
        host.shadowRoot?.querySelector('[data-test="overflow-end-button"]') === null);
});
