...
it('sets `privateSize` on its buttons when `size` is set programmatically', async () => {
  ...
  `);
    host.size = 'small';
    const primaryButton = host.querySelector('glide-core-split-button-primary-button');
    const secondaryButton = host.querySelector('glide-core-split-button-secondary-button');
