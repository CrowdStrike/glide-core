...
it('shows Select All', async () => {
    const host = await fixture(html `<glide-core-dropdown label="Label" multiple select-all open>
      <glide-core-dropdown-option
        label="One"
        selected
      ></glide-core-dropdown-option>
...
it('does not set Select All as selected when no options are provided', async () => {
    const host = await fixture(html `<glide-core-dropdown label="Label" filterable> </glide-core-dropdown>`);
    const selectAll = host.shadowRoot?.querySelector('[data-test="select-all"]');
    expect(selectAll?.selected).to.be.false;
});
it('sets its internal label to `placeholder` when no option is selected', async () => {
    const host = await fixture(html `<glide-core-dropdown label="Label" placeholder="Placeholder" multiple>
      <glide-core-dropdown-option label="One"></glide-core-dropdown-option>
      <glide-core-dropdown-option label="Two"></glide-core-dropdown-option>
    </glide-core-dropdown>`);
...
it('has a "multiselect" icon for each selected option with a value', async () => {
    ...
    const icons = host.shadowRoot?.querySelectorAll('[data-test="multiselect-icon-slot"]');
