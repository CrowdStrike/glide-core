...
it('opens on click', async () => {
    const host = await fixture(html `<glide-core-menu>
      <button slot="target">Target</button>
...
it('opens when `disabled` is set programmatically on its target', async () => {
    const host = await fixture(html `<glide-core-menu open>
...
it('closes when its target clicked', async () => {
    const host = await fixture(html `<glide-core-menu-in-another-component></glide-core-menu-in-another-component>`);
    // Wait for Floating UI.
    await aTimeout(0);
...
it('does not wrap on ArrowDown', async () => {
  ...
  `);
    ...
    await sendKeys({ up: 'Meta' });
    await sendKeys({ press: 'ArrowDown' });
    const option = host.querySelector('glide-core-menu-link:last-of-type');
