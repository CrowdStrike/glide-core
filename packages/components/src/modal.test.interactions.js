...
it('can be closed programmatically', async () => {
    const host = await fixture(html `<glide-core-modal label="Label" open>Content</glide-core-modal>`);
    host.open = false;
    await host.updateComplete;
    const dialog = host.shadowRoot?.querySelector('[data-test="component"]');
...
it('closes when clicked outside', async () => {
    ...
    await click(host.shadowRoot?.querySelector('[data-test="component"]'), 'outside');
    const dialog = host.shadowRoot?.querySelector('[data-test="component"]');
...
it('does not close when `click()` is called on a slotted element', async () => {
    const host = await fixture(html `<glide-core-modal label="Label" open>
      <input />
    </glide-core-modal>`);
...
it('locks scroll on open', async () => {
    await fixture(html `<glide-core-modal label="Label" open>Content</glide-core-modal>`);
