...
it('dispatches a "private-value-change" event when its `value` is set programmatically', async () => {
    const host = await fixture(html `<glide-core-checkbox label="One" value="one"></glide-core-checkbox>`);
...
it('dispatches an "invalid" event on submit when required and unchecked', async () => {
    ...
    const host = await fixture(html `<glide-core-checkbox label="Label" required></glide-core-checkbox>`, { parentNode: form });
...
it('does not dispatch an "invalid" event after `checkValidity()` is called when not required', async () => {
    const form = document.createElement('form');
    const host = await fixture(html `<glide-core-checkbox label="Label"></glide-core-checkbox>`, { parentNode: form });
    const spy = sinon.spy();
