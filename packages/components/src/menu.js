 ...
 * @fires {Event} toggle
 */
let GlideCoreMenu = class GlideCoreMenu extends LitElement {
    constructor() {
        super(...arguments);
        ...
        this.#onDefaultSlotMousedown = (event) => {
        ...
        // An arrow function field instead of a method so `this` is closed over and
        // set to the component instead of `document`.
        this.#onDefaultSlotMouseup = (event) => {
            if (event.target === this.#defaultSlotElementRef.value) {
    ...
    firstUpdated() {
        ...
        this.#targetSlotElementRef.value?.addEventListener('mouseup', this.#onTargetSlotMouseup);
        this.#defaultSlotElementRef.value?.addEventListener('mousedown', this.#onDefaultSlotMousedown);
        this.#defaultSlotElementRef.value?.addEventListener('mouseup', this.#onDefaultSlotMouseup);
    }
    ...
    #onComponentFocusout(event) {
        const isMenuFocused = event.relatedTarget instanceof HTMLElement &&
            this.#shadowRoot?.contains(event.relatedTarget);
        const isOptionsFocused = event.relatedTarget instanceof GlideCoreMenuOptions;
        const isOptionFocused = event.relatedTarget instanceof GlideCoreMenuButton ||
    ...
    #onDefaultSlotFocusin(event) {
        const isButtonOrLink = event.target instanceof GlideCoreMenuButton ||
    ...
    #onDefaultSlotMouseover(event) {
        if ((event.target instanceof GlideCoreMenuLink ||
    ...
    #onOptionsSlotChange() {
        const firstOption = this.#optionElements?.find((option) => !option.disabled);
        if (!this.#activeOption && firstOption) {
            firstOption.privateActive = true;
        }
    }
    #onSlotKeydown(event) {
        const isSpanOrDiv = this.#targetElement instanceof HTMLSpanElement ||
        ...
        if (this.open && this.#activeOption && this.#optionElements) {
            ...
            if ((event.key === 'ArrowDown' && event.metaKey) ||
                ...
                event.key === 'PageDown') {
                event.preventDefault(); // Prevent scroll.
                const lastOption = [...this.#optionElements].findLast((option) => !option.disabled);
    ...
    get #optionElements() {
        ...
        let elements = this.#defaultSlotElementRef.value?.assignedElements()?.at(0)?.children;
        ...
        if (elements) {
            return [...elements].filter((element) => {
                return (element instanceof GlideCoreMenuLink ||
    ...
    #show() {
        ...
        if (this.#targetElement && this.#defaultSlotElementRef.value) {
            this.#cleanUpFloatingUi = autoUpdate(this.#targetElement, this.#defaultSlotElementRef.value, () => {
                (async () => {
                    if (this.#targetElement && this.#defaultSlotElementRef.value) {
                        const { x, y, placement } = await computePosition(this.#targetElement, this.#defaultSlotElementRef.value, {
...
], GlideCoreMenu.prototype, "offset", null);
__decorate([
    property({ reflect: true, type: Boolean })
], GlideCoreMenu.prototype, "open", null);
__decorate([
    property({ reflect: true })
], GlideCoreMenu.prototype, "placement", void 0);
__decorate([
    property({ reflect: true })
], GlideCoreMenu.prototype, "size", null);
__decorate([
    property({ reflect: true })
], GlideCoreMenu.prototype, "version", void 0);
GlideCoreMenu = __decorate([
    customElement('glide-core-menu'),
    final
], GlideCoreMenu);
export default GlideCoreMenu;
