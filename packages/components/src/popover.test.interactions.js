...
it('does not open when opened programmatically and disabled', async () => {
    const host = await fixture(html `<glide-core-popover disabled>
      Popover
      <button slot="target">Target</button>
    </glide-core-popover>`);
    host.open = true;
...
it('closes on Escape', async () => {
    ...
    expect(host.shadowRoot?.querySelector('[data-test="popover"]')?.checkVisibility()).to.be.false;
});
...
it('has `middlewareData.arrow.y` coverage', async () => {
    await fixture(html `<glide-core-popover
      placement="right"
      style=${styleMap({
        alignItems: 'center',
        display: 'flex',
...
it('has `#show()` coverage', async () => {
    const host = await fixture(html `<glide-core-popover placement="right">
      Popover
      <button slot="target">Target</button>
    </glide-core-popover>`);
    host.open = true;
