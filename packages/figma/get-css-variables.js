    ...
    try {
        ...
        for (const file of files) {
            ...
            if (!tokenCollectionName) {
                throw new Error(`Could not determine the collection from the file "${file}".`);
            }
            const fileContent = await readFile(path.join(directory, file), 'utf8');
            const tokens = JSON.parse(fileContent.toString());
 ...
 */
function generateCSSVariablesFromTokens({ prefix, tokens, }) {
    const cssVariables = [];
    for (const [key, value] of Object.entries(tokens)) {
        if (isDesignToken(value)) {
            ...
            // get there.
            cssVariables.push(...generateCSSVariablesFromTokens({
                tokens: value,
