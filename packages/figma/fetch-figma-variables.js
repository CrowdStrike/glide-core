 ...
 */
export default async ({ token, fileId }) => {
    if (!token) {
        throw new Error('"FIGMA_TOKEN" is a required environment variable. See [`CONTRIBUTING.md`](https://github.com/CrowdStrike/glide-core/blob/main/CONTRIBUTING.md#updating-style-variables) for more information.');
    }
    const spinner = yoctoSpinner({ text: 'Fetching Figma variables…' }).start();
    try {
        const response = await fetch(`https://api.figma.com/v1/files/${fileId}/variables/local?scopes=file_variables:read&branch_data=true`, {
            method: 'GET',
            headers: {
                'X-FIGMA-TOKEN': token,
            },
        ...
        if (!response.ok) {
            throw new Error(`The Figma API failed with a ${response.status} error code.`);
        }
        ...
        if (Object.keys(tokens.meta.variableCollections).length === 0) {
            throw new Error('There were no variable collections returned in the response.');
        }
