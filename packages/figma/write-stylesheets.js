 ...
 */
export default async ({ cssVariables, directory, }) => {
    if (!cssVariables) {
    ...
    for (const [collection, variables] of Object.entries(cssVariables)) {
        ...
        try {
            ...
            const cssPath = path.join(process.cwd(), `./${directory}/${collection}.css`);
    ...
    spinner.success(`Stylesheets have been written to the "${directory}" directory.`);
};
