...
export default (meta) => {
    ...
    try {
        const { deletedButReferencedVariableNames, tokens } = buildTokensFromVariables(meta);
        ...
        spinner.error('An error occurred generating tokens from the Figma variables.');
        throw error;
    }
};
function rgbaNumberToHex(value) {
...
function isVariableAlias(object) {
    return (typeof object === 'object' &&
        object !== null &&
        'type' in object &&
        object.type === 'VARIABLE_ALIAS' &&
        'id' in object &&
        typeof object.id === 'string');
}
function getTokenValueFromVariable({ $type, modeId, scopes, variable, variables, }) {
    ...
    if (typeof value === 'object') {
        ...
        throw new Error(`The variable "${JSON.stringify(value)}" is an object, but is unable to be parsed to a hex value or variable alias.`);
    }
...
function buildTokensFromVariables({ variables, variableCollections, }) {
    const tokens = {};
    const deletedButReferencedVariableNames = [];
    for (const variable of Object.values(variables)) {
        ...
        if (!collection) {
            throw new Error(`Could not find a collection with the id of "${variable.variableCollectionId}".`);
        }
        for (const mode of collection.modes) {
            ...
            const modeName = mode.name.toLowerCase() === 'light' ||
            ...
            let tokenGroup = tokens[tokenFileName];
            ...
            for (const [index, part] of parts.entries()) {
                // When we get to the last part, we can stop
                // building up our `tokenGroup` object keys
                // as we're now at the full variable name.
                // We'll write a Design Token to this last
                // part in a moment.
                ...
                if (!tokenGroup[part]) {
                    throw new Error(`Part "${part}" in "${JSON.stringify(tokenGroup)}" does not exist.`);
                }
 ...
 */
function getTokenTypeFromVariable({ scopes, variable, }) {
    switch (variable.resolvedType) {
        ...
        default: {
            throw new Error(`Could not convert Figma's "${variable.resolvedType}" resolvedType to a Design Token $type.`);
        }
    }
}
