 ...
 * Figma's UI.
 */
export default async ({ directory, tokens, }) => {
    const spinner = yoctoSpinner({ text: 'Writing tokens to files…\n' }).start();
    try {
        await mkdir(directory, { recursive: true });
        for (const [fileName, fileContent] of Object.entries(tokens)) {
            await writeFile(`${directory}/${fileName}`, JSON.stringify(fileContent, null, 2));
        }
        spinner.success(`Token files have been written to the "${directory}" directory.`);
    }
    catch (error) {
