...
export default (declaration) => {
    ...
    const properties = [...declaration.members]
        ...
        .filter((member) => {
        const isAttribute = 'attributes' in declaration &&
        ...
        const isIgnore = ['formAssociated', 'shadowRootOptions'].includes(member.name);
        const isPrivate = 'privacy' in member || member.name.startsWith('private');
    ...
    for (const [index, property] of properties.entries()) {
        ...
        tags.push({
            ...
            trailingTrivia(writer) {
                const isCurrentPropertyReadOnly = 'readonly' in property && property.readonly;
                const nextProperty = properties.at(index + 1);
                const isNextPropertyReadOnly = nextProperty && 'readonly' in nextProperty;
                ...
                writer.conditionalNewLine(isCurrentPropertyReadOnly && nextProperty && !isNextPropertyReadOnly);
            },
        });
    }
    return tags;
