...
export default (declaration) => {
    ...
    const methods = [...declaration.members]
        ...
        return (!isPrivate && !isPseudoPrivate && !isIgnore && member.kind === 'method');
    });
    for (const [index, method] of methods.entries()) {
        tags.push({
            ...
            leadingTrivia(writer) {
                ...
                const currentMethodHasReturn = 'return' in method && method.return?.type.text !== 'void';
                const previousMethod = methods.at(index - 1);
                const previousMethodHasParameters = previousMethod && 'parameters' in previousMethod;
                const previousMethodHasReturn = previousMethod &&
                    'return' in previousMethod &&
                    previousMethod.return?.type.text !== 'void';
                if (currentMethodHasParameters ||
